...
首页文章卡片会打开独立阅读页，展示封面、完整正文、阅读信息及上一篇/下一篇。每篇文章都有可复制分享的网址，例如 `article.html?id=self-kindness`；刷新或直接打开这个地址也能阅读。

在编辑页的“心理学分享 → 文章”中修改全文。新增文章会自动生成独立的链接标识，排序和改标题不会改变链接；不要随意修改已发布文章的链接标识，否则之前分享的链接将失效。预览里点击文章卡片即可进入详情页，仍会显示本次未发布的修改。删除文章后，原链接显示返回文章列表的提示。

“咨询师简介 → 资质背景”提供独立的显示开关、标题、说明和分类列表。教育背景、主要受训、短程进修、专业支持的标题和项目均可修改、增删与排序。当前资料整理自所有者已上传的内容，未新增资质或认证声明。

旧版内容文件仍可导入，原有文案会保留。缺少资质背景设置的旧文件会将这个新区域设为隐藏，避免自动加入未填写的个人资料；可在编辑器中手动开启并补充。

品牌名称为“光渡心理”，咨询师为“时隐”。专业背景整理自所有者提供的资料；预约仍为展示功能，收费与服务规则以所有者确认的信息为准。
