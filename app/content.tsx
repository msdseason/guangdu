...
export function Closing(){const {site,t}=useSite(),c=site.结尾;return <section className="closing"><span className="tiny-sprout">{c.符号}</span><h2>{t(c.标题)}</h2><p>{t(c.正文)}</p><a className="primary-link" href={safeLink(c.按钮地址)}>{t(c.按钮文字)}<ArrowUpRight size={17}/></a></section>}
